...
export class ScreenUtils {
  ...
  public static calculateScreenMetrics(
    defaultWidth: number,
    ...
    let dimensionsOppositeForLandscape: boolean =
      this.screenMetrics.windowWidth < this.screenMetrics.windowHeight &&
      orientation === ScreenOrientation.LANDSCAPE;
    let dimensionsOppositeForPortrait: boolean =
      this.screenMetrics.windowHeight < this.screenMetrics.windowWidth &&
      orientation === ScreenOrientation.PORTRAIT;

    if (dimensionsOppositeForLandscape || dimensionsOppositeForPortrait) {
      [this.screenMetrics.windowWidth, this.screenMetrics.windowHeight] = [
        this.screenMetrics.windowHeight,
    ...
    if (!maxGameWidth || !maxGameHeight) {
      if (orientation === ScreenOrientation.LANDSCAPE) {
        this.screenMetrics.maxGameWidth = Math.round(
          this.screenMetrics.defaultGameWidth * (MAX_GAME_WIDTH / DEFAULT_GAME_WIDTH)
        );
        this.screenMetrics.maxGameHeight = Math.round(
          this.screenMetrics.defaultGameHeight * (MAX_GAME_HEIGHT / DEFAULT_GAME_HEIGHT)
        );
      } else {
        this.screenMetrics.maxGameWidth = Math.round(
          this.screenMetrics.defaultGameWidth * (MAX_GAME_HEIGHT / DEFAULT_GAME_HEIGHT)
        );
        this.screenMetrics.maxGameHeight = Math.round(
          this.screenMetrics.defaultGameHeight * (MAX_GAME_WIDTH / DEFAULT_GAME_WIDTH)
        );
      }
    } else {
    ...
    let defaultAspectRatio: number =
      orientation === ScreenOrientation.LANDSCAPE
        ...
        : DEFAULT_GAME_HEIGHT / DEFAULT_GAME_WIDTH;
    ...
    if (windowAspectRatio > defaultAspectRatio) {
      ...
      this.screenMetrics.gameWidth =
        Math.ceil(this.screenMetrics.gameHeight * windowAspectRatio * 0.5) * 2;
      this.screenMetrics.gameWidth = Math.min(
        this.screenMetrics.gameWidth,
        this.screenMetrics.maxGameWidth
      );

      this.screenMetrics.offsetX =
        (this.screenMetrics.gameWidth - this.screenMetrics.defaultGameWidth) * 0.5;
      ...
      this.screenMetrics.gameHeight =
        Math.ceil(this.screenMetrics.gameWidth / windowAspectRatio * 0.5) * 2;
      this.screenMetrics.gameHeight = Math.min(
        this.screenMetrics.gameHeight,
        this.screenMetrics.maxGameHeight
      );

      this.screenMetrics.offsetX = 0;
      this.screenMetrics.offsetY =
        (this.screenMetrics.gameHeight - this.screenMetrics.defaultGameHeight) * 0.5;
    }
